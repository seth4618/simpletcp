...
TCPServer.prototype.onConnect = function(c) {
    ...
    // setup handlers
    c.on('close', function() { connObject.onEnd();});
...
function TCPConnection(server, socket)
{
    ...
    if (this.verbose) 
        Util.info("Set up a new connection for "+this.server.name+" with "+this.getRemote());
...
JSONConnection.prototype.lineReceived = function(data) 
{
    ...
    try {
        ...
        if (this.verbose) 
            Util.error(this.server.name+"@"+this.getRemote()+" json error:"+err);
        this.emit('malformed', data);
        return;
    }
