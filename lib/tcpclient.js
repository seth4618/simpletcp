...
function TCPClient(name, host, port, connfunc) 
{
    ...
    if (this.port == undefined) throw new Error("port is not defined");
...
TCPClient.prototype.open = function() 
{
    ...
    if (this.verbose)
        TCPClient.info("Trying to establish a connection to "+this.name+" server @"+this.host+":"+this.port);

    var me = this;
    try {
        this.client = Net.connect(this.port, 
                                  this.host, 
                                  function () {
                                      me.status = 1;
                                      if (TCPClient.verbose) TCPClient.info("return from Net.connect for "+me.name);
                                      me.emit('connect');
                                  });
        ...
        this.client.on('err', function (err) { me.errorReceived(err);});
    } catch (err) {
...
TCPClient.prototype.inputReceived = function(data)
{
    ...
    this.buffer += str;
    if (TCPClient.verbose) TCPClient.info(this.name+": input: ["+this.buffer+"]");
    var nl = this.buffer.indexOf("\n");
    do {
        if (nl == -1) {
            if (TCPClient.verbose) TCPClient.info(this.name+':partial line recieved ['+str+']');
            return;
        }
        ...
        if (nl+1 == len) {
            ...
            if (TCPClient.verbose) TCPClient.info(this.name+':bufffer ['+this.buffer+']');
        }
...
{
    TCPClient.error(this.name+':error recieved: '+err); 
    this.errorCount++;
    this.emit('err', err);
};
