...
/** @type {boolean} */ TCPClient.prototype.ignoreRefusal;
...
/**
 * setIgnoreRefusal
 * If noone is listening to 'error', then
 *    true -> don't report refusal errors since we retry anyway.
 *    false -> report them by emitting error event
 * If listening to 'error', then let them decide
 *
 * @param {boolean=} flag		not defined means toggle
 * @return {boolean}
 **/
TCPClient.prototype.setIgnoreRefusal = function(flag)
{
    if (flag == undefined) flag = !this.ignoreRefusal;
    this.ignoreRefusal = flag;
    return this.ignoreRefusal;
};
...
TCPClient.prototype.errorReceived = function(err)
{
    ...
    if ((any.length == 0)&&(this.ignoreRefusal && (err.code == 'ECONNREFUSED'))) return;

    err.tcpclient = this.name+':'+this.host; // give the user some extra info if they forgot to listen to error
    this.emit('error', err);
};
